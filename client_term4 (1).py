...
def set_system_volume(level: int) -> None:
    ...
    if system == "Windows":
        ...
        # Новые версии pycaw возвращают AudioDevice-обёртку без метода Activate;
        # реальный IMMDevice COM-объект лежит в ._dev. Старые версии возвращают
        # COM-объект напрямую — у него Activate есть сразу.
        ...
        try:
            ...
            async with websockets.connect(SERVER_URL, max_size=None) as ws:
                ...
                try:
                    ...
                    # Keep listening even if hello failed; server commands may still work.
